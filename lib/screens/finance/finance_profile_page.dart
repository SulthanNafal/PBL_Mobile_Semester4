...
class _FinanceProfilePageState extends State<FinanceProfilePage> {
  ...
  Future<void> _fetchProfile() async {
    try {
      ...
      if (user == null) return;
      final data = await supabase.schema('ursaevent').from('users').select().eq('id', user.id).maybeSingle();
          ...
          children: [
            Container(
              ...
              child: const CircleAvatar(
                ...
                child: Icon(Icons.account_balance_wallet_outlined, size: 48, color: Color(0xFFD32F2F)),
              ),
            ),
            const SizedBox(height: 16),
            Text(_userData?['username'] ?? 'Finance', style: const TextStyle(fontSize: 20, fontWeight: FontWeight.bold)),
            const SizedBox(height: 4),
            Container(
              padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 4),
              decoration: BoxDecoration(color: Colors.green.shade50, borderRadius: BorderRadius.circular(12)),
              child: Text('Finance', style: TextStyle(color: Colors.green.shade700, fontSize: 12, fontWeight: FontWeight.w600)),
            ),
            ...
            Container(
              ...
              child: Column(
                ...
                children: [
                  ...
                  const SizedBox(height: 12),
                  _profileRow(Icons.shield_outlined, 'Level', _userData?['level'] ?? 'finance'),
                ],
              ),
            ),
